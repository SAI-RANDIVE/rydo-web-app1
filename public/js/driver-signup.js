document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateStep(step) {
        ...
        requiredFields.forEach(field => {
            ...
            } else if (field.name === 'languages_known' || field.name === 'vehicle_types') {
                // For checkbox groups (languages, vehicle types)
    ...
    function populateReviewSection() {
        ...
        const uploadFields = [
            ...
            { id: 'license_photo_front', label: 'License Front Photo' },
            { id: 'license_photo_back', label: 'License Back Photo' },
        ...
        if (educationLevel.selectedIndex !== -1) {
            reviewProfessional.innerHTML += `<p><strong>Education Level:</strong> ${educationLevel.options[educationLevel.selectedIndex].text}</p>`;
        }
        ...
        if (experience) {
            reviewProfessional.innerHTML += `<p><strong>Driving Experience:</strong> ${experience} years</p>`;
        }
        
        // Vehicle Types
        const vehicleCheckboxes = document.querySelectorAll('input[name="vehicle_types"]:checked');
        if (vehicleCheckboxes.length > 0) {
            const vehicles = Array.from(vehicleCheckboxes).map(cb => cb.value).join(', ');
            reviewProfessional.innerHTML += `<p><strong>Vehicle Types:</strong> ${vehicles}</p>`;
        }
        ...
        // Preferred Locations
        const locations = document.getElementById('preferred_locations').value;
        if (locations) {
            reviewProfessional.innerHTML += `<p><strong>Preferred Locations:</strong> ${locations}</p>`;
        }
        ...
        if (upiId) {
            reviewProfessional.innerHTML += `<p><strong>UPI ID:</strong> ${upiId}</p>`;
        }
    }
    ...
    if (driverSignupForm) {
        driverSignupForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            const vehicleCheckboxes = document.querySelectorAll('input[name="vehicle_types"]:checked');
            const vehicles = Array.from(vehicleCheckboxes).map(cb => cb.value).join(',');
            formData.set('vehicle_types', vehicles);
            ...
            try {
                ...
                const response = await fetch('/auth/driver-signup', {
                    method: 'POST',
                    body: formData
                });
